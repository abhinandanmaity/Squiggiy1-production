                ...
                <div className="flex flex-wrap justify-around">
                    ...
                    {products && Object.keys(products).map((item) => {
                        ...
                        return (
                            ...
                            <div key={products[item]._id} className=" xl:w-1/4 md:w-1/2 xs:w-1/2 p-8 hover:scale-110 ease-in duration-200">
                                <div className=" bg-gray-100 p-2 rounded-xl  shadow-2xl ">
                                    <Link href={`/product/${products[item].slug}`}><a>
                                        ...
                                        <div className="mx-10 items-start">
                                            ...
                                            <h2 className="sm:text-xs text-xxs text-gray-400 font-medium title-font pb-2">dairy</h2>
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        ...
        if (item.avalibleQty > 0) {
            ...
            if (item.slug in dairy) {

                if (!dairy[item.slug].quantity.includes(item.quantity) && item.avalibleQty > 0) {
                    dairy[item.slug].quantity.push(item.quantity)
                }
                if (!dairy[item.slug].mesure.includes(item.mesure) && item.avalibleQty > 0) {
                    dairy[item.slug].mesure.push(item.mesure)
                }
            } else {
                dairy[item.slug] = JSON.parse(JSON.stringify(item))
                if (item.avalibleQty > 0) {
                    dairy[item.slug].quantity = [item.quantity]
                    dairy[item.slug].mesure = [item.mesure]
                }
            }
            count = 1;
        }
    ...
    if (count == 1) {
        ...
        return {
            props: { products: JSON.parse(JSON.stringify(dairy)) }
        }
    } else {
